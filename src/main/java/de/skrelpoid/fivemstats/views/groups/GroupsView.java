...
public class GroupsView extends Div implements BeforeEnterObserver {
    ...
    public GroupsView(final GroupService groupService, final PlayerService playerService) {
        ...
        delete.addClickListener(e -> {
        	if (!e.isShiftKey()) {
        		Notification.show("Hold Down Shift Key while Pressing Delete to Confirm Deletion", 3000, Position.MIDDLE);
        		return;
        	}
        	if (this.group != null) {
    ...
    private void createEditorLayout(final SplitLayout splitLayout) {
        ...
        players.addCustomValueSetListener(event -> {
        	...
        	final Specification<Player> spec = (r, q, s) -> playerService.buildSearchPredicate(customValue, r, s, null);
        	final List<Player> foundPlayers = playerService.list(spec);
        	if (foundPlayers.isEmpty()) {
        		// show notification
        		final Notification notification =  Notification.show("No players found for searched value: " + customValue, 3000, Position.MIDDLE);
        		notification.addThemeVariants(NotificationVariant.LUMO_ERROR);
        	} else {
        		players.select(foundPlayers);
        		final String foundStr = foundPlayers.stream().map(Player::toString).collect(joining(", "));
        		final Notification notification =  Notification.show("Added players: " + foundStr, 3000, Position.BOTTOM_START);

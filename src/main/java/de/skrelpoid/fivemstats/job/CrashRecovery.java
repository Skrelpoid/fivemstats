...
/**
 * If the application crashed (or shut down when people were still logged in and they couldn't be
 * logged out), this class will make sure on startup to apply a sensible log out time, which will
 * either be the log in time + 1 min or the last active time of the service logged in the heartbeat
 * table
 *
...
public class CrashRecovery {
	...
	void init() {
		...
		final Heartbeat lastKnownHeartbeat =
				heartbeatRepository.findAll().stream().findFirst().orElse(new Heartbeat(LocalDateTime.MIN));
		...
		for (final PlayerLog playerLog : activeLogs) {
			...
			if (logInTime.isBefore(heartbeatTime)) {
				...
				final var logInPlus1 = logInTime.plusMinutes(1);
				final var logOutTime = logInPlus1.isAfter(now) ? now : logInPlus1;
